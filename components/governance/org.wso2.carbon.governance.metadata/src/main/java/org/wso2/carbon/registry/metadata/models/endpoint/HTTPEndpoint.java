...
import org.wso2.carbon.registry.metadata.models.version.EndpointVersion;

import java.util.ArrayList;
import java.util.List;
import java.util.Map;

public class HTTPEndpoint extends Base {
...
//  Type specific attributes goes here
    ...
    public HTTPEndpoint(Registry registry, String name) throws MetadataException {
        super(mediaType, name, registry);
    }

    public HTTPEndpoint(Registry registry, String name, VersionBase version) throws MetadataException {
        super(mediaType, name, registry,version);
    }

    public HTTPEndpoint(Registry registry, String name, String uuid, Map<String, List<String>> propertyBag, Map<String, List<String>> attributeMap) throws MetadataException {
        super(mediaType,name, uuid, propertyBag,attributeMap, registry);
    }

    public EndpointVersion newVersion(String key) throws MetadataException {
        EndpointVersion v = new EndpointVersion(registry, key);
        v.setBaseUUID(uuid);
        v.setBaseName(name);
        return v;
    }
    ...
    public static void add(Registry registry, Base metadata) throws MetadataException {
        if (((HTTPEndpoint) metadata).baseVersion == null) {
            add(registry, metadata,
                    generateMetadataStoragePath(metadata.getName(), metadata.getRootStoragePath()));
        } else {
            add(registry, metadata,
                    generateMetadataStoragePath(metadata.getName(), metadata.getRootStoragePath()));
            EndpointVersion.add(registry, ((HTTPEndpoint) metadata).baseVersion);
        }
    }
     ...
     */
    public static HTTPEndpoint[] getAll(Registry registry) throws MetadataException {
     ...
     * @return
     */
    public static HTTPEndpoint[] find(Registry registry, Map<String, String> criteria) throws MetadataException {
     ...
     */
    public static HTTPEndpoint get(Registry registry, String uuid) throws MetadataException {
        return (HTTPEndpoint) get(registry, uuid, mediaType);
    }
