...
public class HTTPEndpointProvider implements BaseProvider {

    private static final Log log = LogFactory.getLog(HTTPEndpointProvider.class);

    private final String mediaType;
    private final String versionMediaType;


    public HTTPEndpointProvider(String mediaType, String versionMediaType) {
    ...
    private HTTPEndpoint getFilledBean(OMElement root, Map<String, List<String>> propBag, Registry registry) throws MetadataException {
        ...
        return new HTTPEndpoint(registry, name, uuid, propBag, attributeMap);
    }

    private void createAttributesContent(HTTPEndpoint httpEndpoint, OMElement element) throws MetadataException {
        OMFactory factory = OMAbstractFactory.getOMFactory();
        OMElement uuid = factory.createOMElement(new QName(Constants.ATTRIBUTE_UUID));
        uuid.setText(httpEndpoint.getUUID());
        OMElement name = factory.createOMElement(new QName(Constants.ATTRIBUTE_METADATA_NAME));
        name.setText(httpEndpoint.getName());

        OMElement endpoint = factory.createOMElement(new QName(HTTPEndpoint.KEY_URL));
        endpoint.setText(httpEndpoint.getUrl());

        element.addChild(uuid);
        element.addChild(name);
        element.addChild(endpoint);

        buildGenericAttributes(httpEndpoint, element, factory);
    }

    private void buildGenericAttributes(HTTPEndpoint metadata, OMElement attributeRoot, OMFactory factory) {
    ...
    private void createPropertiesContent(HTTPEndpoint httpEndpoint, OMElement element) {
        OMFactory factory = OMAbstractFactory.getOMFactory();
        for (Map.Entry<String, List<String>> entry : httpEndpoint.getPropertyBag().entrySet()) {
    ...
    private OMElement getGeneratedMetadataOMElement(Base metadata) throws MetadataException {
        ...
        createAttributesContent((HTTPEndpoint) metadata, attributes);
        createPropertiesContent((HTTPEndpoint) metadata, properties);
