...
import org.wso2.carbon.registry.metadata.models.version.GenericVersion;
...
public class GenericVersionProvider implements VersionBaseProvider {

    private static final Log log = LogFactory.getLog(GenericVersionProvider.class);

    private String mediaType;


    public GenericVersionProvider(String mediaType) {
    ...
    public Resource buildResource(VersionBase metadata, Resource resource) throws MetadataException {
        ...
        createAttributesContent((GenericVersion) metadata, attributes);
        createPropertiesContent((GenericVersion) metadata, properties);
    ...
    @Override
    public GenericVersion get(Resource resource, Registry registry) throws MetadataException {
        try {
    ...
    private GenericVersion getFilledBean(OMElement root, Map<String, List<String>> propBag, Registry registry) throws MetadataException {
        ...
        return new GenericVersion(registry, name, uuid, baseName, baseUUID, propBag, attributeMap);
    }


    private void createAttributesContent(GenericVersion genericVersion, OMElement element) throws MetadataException {

        OMFactory factory = OMAbstractFactory.getOMFactory();

        OMElement uuid = factory.createOMElement(new QName(Constants.ATTRIBUTE_UUID));
        uuid.setText(genericVersion.getUUID());

        OMElement name = factory.createOMElement(new QName(Constants.ATTRIBUTE_METADATA_NAME));
        name.setText(genericVersion.getName());

        OMElement baseName = factory.createOMElement(new QName(Constants.ATTRIBUTE_METADATA_BASE_NAME));
        baseName.setText(genericVersion.getBaseName());

        OMElement baseUUID = factory.createOMElement(new QName(Constants.ATTRIBUTE_BASE_UUID));
        baseUUID.setText(genericVersion.getBaseUUID());
    ...
    private void createPropertiesContent(GenericVersion genericVersion, OMElement element) {
        OMFactory factory = OMAbstractFactory.getOMFactory();
        for (Map.Entry<String, List<String>> entry : genericVersion.getPropertyBag().entrySet()) {
