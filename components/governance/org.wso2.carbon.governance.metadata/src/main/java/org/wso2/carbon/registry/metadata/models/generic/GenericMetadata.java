...
//  Variables defined for the internal implementation
    ...
    private GenericVersion baseVersion = null;
    ...
    public GenericVersion newVersion(String key) throws MetadataException {
        GenericVersion v = new GenericVersion(registry, key);
        v.setBaseUUID(uuid);
        v.setBaseName(name);
        return v;
    }
    ...
    public static void add(Registry registry, Base metadata) throws MetadataException {
        if (((GenericMetadata) metadata).baseVersion == null) {
            ...
            add(registry, metadata,
                    generateMetadataStoragePath(metadata.getName(), metadata.getRootStoragePath()));
            GenericVersion.add(registry, ((GenericMetadata) metadata).baseVersion);
        }
    }
