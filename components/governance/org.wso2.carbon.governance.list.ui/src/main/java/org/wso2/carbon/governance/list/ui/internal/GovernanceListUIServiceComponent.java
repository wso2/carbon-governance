...
@Component(
         name = "org.wso2.carbon.governance.list.ui", 
         immediate = true)
public class GovernanceListUIServiceComponent {
                ...
                if (CarbonUIUtil.isUserAuthorized(request, "/permission/admin/manage/resources/ws-api")) {
                    ...
                    try {
                        ...
                        for (GovernanceArtifactConfiguration configuration : configurations) {
                            ...
                            if (realmService.getTenantUserRealm(realmService.getTenantManager().getTenantId(s1)).getAuthorizationManager().isUserAuthorized(s, configurationPath, ActionConstants.PUT) || registry.resourceExists(layoutStoragePath)) {
                        ...
                        session.setAttribute(MenuAdminClient.USER_CUSTOM_MENU_ITEMS, userCustomMenuItemsList.toArray(new Menu[userCustomMenuItemsList.size()]));
    ...
    private void buildViewUI(GovernanceArtifactConfiguration configuration, Map<String, String> viewUIMap, String key) {
        ...
        if (singularLabel == null || pluralLabel == null) {
            log.error("The singular label and plural label have not " + "been defined for the artifact type: " + key);
        } else {
    ...
    *  */
        ...
        try {
            ...
            if (resultNodes != null && resultNodes.size() > 0) {
                ...
                if (lifecycleParentName != null && lifecycleName != null) {
                    return convertName(lifecycleParentName.split(" ")) + "_" + convertName(lifecycleName.split(" "));
                }
            }
        } catch (OMException e) {
    ...
    private void buildMenuList(HttpServletRequest request, GovernanceArtifactConfiguration configuration, List<Menu> menuList, String key) {
        ...
        if (singularLabel == null || pluralLabel == null) {
            ...
            if (CarbonUIUtil.isUserAuthorized(request, "/permission/admin/manage/resources/govern/" + key + "/add") && CarbonUIUtil.isUserAuthorized(request, "/permission/admin/manage/resources/browse") && CarbonUIUtil.isUserAuthorized(request, "/permission/admin/manage/resources/ws-api")) {
            ...
            if (CarbonUIUtil.isUserAuthorized(request, "/permission/admin/manage/resources/govern/" + key + "/list") && CarbonUIUtil.isUserAuthorized(request, "/permission/admin/manage/resources/ws-api")) {
