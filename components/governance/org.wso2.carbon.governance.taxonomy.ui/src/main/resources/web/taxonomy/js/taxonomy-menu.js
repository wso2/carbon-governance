...
function childMenuItemClicked(e, e2, objx) {
    ...
    jQuery("#treePane").load(url, null, function (responseText, status, XMLHttpRequest) {
        if (status != "success") {
            CARBON.showErrorDialog(org_wso2_carbon_governance_lcm_ui_jsi18n["mediator.design.load.error"]);
        } else {
            initMediators();
...
function deleteCallback(e, obj) {
    ...
    CARBON.showConfirmationDialog(org_wso2_carbon_governance_lcm_ui_jsi18n["mediator.delete.confirmation"], function () {
        var url = "design_sequence-ajaxprocessor.jsp?mediatorID="
...
function addNodesCallback(e, obj) {
    ...
    for (var i = 0; i < g; i++) {
        ...
        for (var j = 0; j < rootMenuItem.length; j++) {
            rootMenuItem[j].cfg.setProperty("onclick", {
                fn: childMenuItemClicked, obj: [rootMenuItem[j].id, clickedMediatorNodeId, obj[2]]
            })
        }
    }
